...
int isAsciiDigit(int x) {
  int highNibble = (x >> 4) << 4;
  int maxRange = !(highNibble^0x30);
  int leadingBitInLastNibble = !(x & 8);
  int testOtherCases = (leadingBitInLastNibble | !(x & 0x06));
  return maxRange & testOtherCases;
}
/* 
 ...
 *   Examples: bitParity(5) = 0straightline, bitParity(7) = 1
 *   Legal ops: ! ~ & ^ | + << >>
 *   Max ops: 20
 *   Rating: 4
 */
